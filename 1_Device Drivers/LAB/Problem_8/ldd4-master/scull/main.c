...
int scull_read_procmem(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
		...
		if (mutex_lock_interruptible(&d->mutex))
			return -ERESTARTSYS;
		seq_printf(s, "\ninitial quantum for device %i: ",d->quantum);
		d->quantum = 2000;
		seq_printf(s, "\nChanged quantum for device %i: ",d->quantum);
...
{
	proc_create_data("change quantum", 0 /* default mode */,
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("change quantum", NULL /* parent dir */);
